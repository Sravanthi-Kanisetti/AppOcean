...
📂 CSV Source Files
        ...
        ▼
┌──────────────────┐
│  Staging Layer    │  ← Raw data loaded via Stored Procedure (Staging.load_data)
│  (Staging schema) │  ← NVARCHAR columns — accepts everything safely
└────────┬──────────┘
         │
         ▼  [Data Profiling → Data Cleaning]
         │
┌──────────────────┐
│   DW Layer        │  ← Cleaned data loaded via Stored Procedure (DW.load_data)
...
├── 📂 03_Data_Profiling/
│     └── Data_Profiling.sql
│         └── DISTINCT + COLLATE, COUNT NULL checks, MAX/MIN/AVG for all columns
│
├── 📂 04_Data_Cleaning/
│     └── Cleaning_Logic.sql
│         └── SELECT-first cleaning logic verified before being moved into SP2
│
├── 📂 05_DW_Layer/
│     ├── 01_ddl_dw.sql
...
├── 📂 08_Data_Marts/
│     └── DataMarts.sql
│         └── 6 Views: Marketing_Mart, Finance_Mart, Product_Mart,
│                       Regional_Mart, Device_Mart, Executive_Mart
│
...
-- ⚠️ Before executing: update CSV file paths inside the SP to your local folder
-- Then execute:
...
| is_weekend | 'True\r'/'False\r' string with hidden carriage return | Converted to BIT (1/0) via REPLACE(col, CHAR(13), '') |
| install_source | NULL values | Replaced with 'Unknown' |
| app_version | NULL / mixed v-prefix | TRIM + replaced NULLs with 'Unknown' |
...
| Device_Mart | Tech/Engineering | device_type, os_version, manufacturer, screen_size_inch |
...
| NULL → Unknown in dims | Business queries GROUP BY dimensions — NULL causes confusing, fragmented results |
| NULL kept in fact measures | revenue NULL = event didn't generate revenue — meaningful, not missing |
| Views for Data Marts | No data duplication, always fresh, true Kimball approach (vs Medallion architecture) |
